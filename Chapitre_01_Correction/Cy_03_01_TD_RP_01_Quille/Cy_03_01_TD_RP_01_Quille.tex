...
\subparagraph{}\textit{Montrer, en précisant la ou les exigences mises en défaut, que le cahier des charges n’est pas respecté au niveau des critères « vérifiables ».}
\ifprof
\begin{corrige} ~\\
...
\subparagraph{}\textit{Pour $K_{\text{COR}} =1$, en faisant correspondre la pulsation de résonance $\omega_r$ de $H_{\text{BO}} $ à $\omega^{*}$ :
\begin{itemize}
