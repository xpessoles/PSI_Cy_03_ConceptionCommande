%%%% Paramétrage du TD %%%%
\def\xxactivite{TD 4 \ifprof -- Corrigé \else \fi} % \normalsize \vspace{-.4cm}
...
\subparagraph{}\textit{Tracer le diagramme de Bode de la FTBO du système pour $K_C=1$ et donner les marges. Le cahier des charges est-il vérifié ?}
\ifprof
\begin{corrige}~\\
...
\subparagraph{}\textit{Déterminer la fonction de transfert en boucle ouverte du système $\text{FTBO}(p)=\dfrac{X(p)}{\varepsilon_x(p)}$ en supposant que $F_p=0$.}
\ifprof
\begin{corrige}~\\
